...
开发一款基于 **Tauri (Rust)** 的图形化桌面应用，封装 `cloudflared` 二进制文件。服务提供方持有自有域名 `cloudplay.app`，通过泛域名解析和 Cloudflare Tunnel，为每位用户分配唯一的子域名（如 `player123.cloudplay.app`），实现"开服者一键启动，联机者一键连接"的便捷体验。
...
此配置确保所有子域名（`*.cloudplay.app`）的请求均被转发至同一持久隧道，无需为每位用户手动配置 DNS 记录。

### 3.2 子域名分配逻辑

- **格式**：`{用户自定义ID}.cloudplay.app`
- **层级说明**：虽然这是三级域名，但 Cloudflare 免费套餐的通配符证书（`*.cloudplay.app`）完全覆盖，SSL 握手正常。
...
| 子域名 | 用途 | 托管于 |
| :--- | :--- | :--- |
| `*.cloudplay.app` | 游戏隧道泛解析 | Cloudflare Tunnel |
| `cloudplay.app` / `www.cloudplay.app` | 官网及下载页 | Cloudflare Pages |
| `api.cloudplay.app` | Token 颁发 API | Cloudflare Workers |
| `cdn.cloudplay.app` | 安装包等静态资源（可选） | Cloudflare R2 |
...
**端点**：`POST https://api.cloudplay.app/api/token`
...
```json
{
  "success": true,
  "data": {
    "hostname": "john123.cloudplay.app",
...
app.use('*', cors({
  origin: ['https://cloudplay.app', 'https://www.cloudplay.app'],
...
1. 玩家输入开服者提供的地址（如 `john123.cloudplay.app`）。
2. 点击"连接游戏"。
3. Rust 后端执行命令：
    ```bash
    cloudflared access tcp --hostname john123.cloudplay.app --url localhost:25566
    ```
...
- 域名绑定：`cloudplay.app` 指向 Pages 项目。
...
| 阶段 | 任务 | 优先级 | 状态 |
| :--- | :--- | :--- | :--- |
| **阶段一：基础设施** | 注册 `cloudplay.app`，接入 Cloudflare | P0 | 待办 |
| | 配置泛域名解析 `*.cloudplay.app` | P0 | 待办 |
